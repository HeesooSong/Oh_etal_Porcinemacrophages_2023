...
circular_barplot <- function(data, datatype, analysis, celltype, title){
  ...
  CircularPlot <- ggplot(plot_df, aes(x=as.factor(id), y=value, fill=group)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
    ...
    geom_segment(data=grid_data, aes(x = end, y = 12, xend = start, yend = 12), colour = "grey", alpha=1, size=0.5 , inherit.aes = FALSE ) +
    geom_segment(data=grid_data, aes(x = end, y = 9, xend = start, yend = 9), colour = "grey", alpha=1, size=0.5 , inherit.aes = FALSE ) +
    geom_segment(data=grid_data, aes(x = end, y = 6, xend = start, yend = 6), colour = "grey", alpha=1, size=0.5 , inherit.aes = FALSE ) +
    geom_segment(data=grid_data, aes(x = end, y = 3, xend = start, yend = 3), colour = "grey", alpha=1, size=0.5 , inherit.aes = FALSE ) +
    
    # Add text showing the value of each 100/75/50/25 lines
    annotate("text", x = rep(max(plot_df$id),4), y = c(3, 6, 9, 12), label = c("3", "6", "9", "12") , color="grey", size=6 , angle=0, fontface="bold", hjust=1) +
    ...
    coord_polar() + 
    geom_text(data=label_data, aes(x=id, y=value+0.1, label=geneName_pig, hjust=hjust), color="black", fontface="bold",alpha=1, size=4, angle= label_data$angle, inherit.aes = FALSE ) +
    annotate("text", label = title, x = 10, y = -10, size = 9, fontface="bold", colour = "black") +

    # Add base line information
    geom_segment(data=base_data, aes(x = start, y = -0.5, xend = end, yend = -0.5), colour = "black", alpha=0.8, size=1.5 , inherit.aes = FALSE )  +
    geom_text(data=base_data, aes(x = group_title, y = -1.5, label=group), hjust= 0.5, colour = "black", alpha=1, size=10, fontface="bold", inherit.aes = FALSE)
...
## 1) MF
circular_barplot(MF_ABC, "MF", "AvsBvsC", "A", expression(atop("FACS-RNAseq", paste("NaSn"^"-"))))
circular_barplot(MF_ABC, "MF", "AvsBvsC", "B", expression(atop("FACS-RNAseq", paste("NaSn"^"+"))))
circular_barplot(MF_ABC, "MF", "AvsBvsC", "C", expression(atop("FACS-RNAseq", paste("LuSn"^"+"))))

circular_barplot(MF_LvsN, "MF", "LungvsNose", "lung", "FACS-RNAseq \n Lung macrophages")
circular_barplot(MF_LvsN, "MF", "LungvsNose", "nose", "FACS-RNAseq \n Nasal macrophages")

## 2) LCM
circular_barplot_legend(LCM_ABC, "LCM", "AvsBvsC", "A", expression(atop("LCM-RNAseq", paste("NaSn"^"-"))))
circular_barplot(LCM_ABC, "LCM", "AvsBvsC", "B", expression(atop("LCM-RNAseq", paste("NaSn"^"+"))))
circular_barplot(LCM_ABC, "LCM", "AvsBvsC", "C", expression(atop("LCM-RNAseq", paste("LuSn"^"+"))))
