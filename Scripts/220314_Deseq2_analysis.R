...
plotPCA(vsd, intgroup=c("condition")) +
  ...
  xlim(-40, 65) +
  ylim(-30, 35) +
  scale_color_discrete(labels = c(bquote("NaSn"^"low"), bquote("NaSn"^"mid"), bquote("LuSn"^"high"))) +
...
# Volcano plot
test <- merge(ENS_NCBI, as.data.frame(B_NasalSnPos_vs_A_NasalSnNeg), by.x = "Ensembl_GeneID_pig", by.y =0)
...
threshold_A <- sort(-log10(test$padj) * test$log2FoldChange, decreasing = TRUE)[6]
threshold_B <- sort(-log10(test$padj) * test$log2FoldChange, decreasing = FALSE)[6]
...
p <- ggplot(data = test, 
  ...
  scale_color_manual(values=c("DEGs in NaSn-"="#F8766D", "DEGs in NaSn+"="#00BA38", "non-DEGs"="grey"),
                     labels = c(bquote("DEGs in NaSn"^"low"), bquote("DEGs in NaSn"^"mid"), "non-DEGs"))+
  geom_vline(xintercept=c(-1,1),lty=4,col="black",lwd=0.5) +
  geom_hline(yintercept = 1.301,lty=4,col="black",lwd=0.5) +
  labs(x="log2(fold change)",
       y="-log10 (adj.p-value)",
       title = expression("NaSn"^"low"*" vs NaSn"^"mid"))  +
  theme_bw()+
  theme(plot.title = element_text(size = 20, hjust = 0.5, colour = "#D39200"), 
        ...
        legend.key.size = unit(1, 'cm'))+
  geom_text_repel(
    data=test %>% filter((-log10(padj)*log2FoldChange) > threshold_A | (-log10(padj)*log2FoldChange) < threshold_B), # Filter data first
...
p <- ggplot(data = test, 
            aes(x = log2FoldChange, 
                ...
                colour=expression)) +
  geom_point(alpha=0.8, size=2) +
  scale_color_manual(values=c("DEGs in NaSn-"="#F8766D", "DEGs in LuSn+"="#619CFF", "non-DEGs"="grey"),
                     labels = c(bquote("DEGs in NaSn"^"low"), bquote("DEGs in LuSn"^"high"), "non-DEGs"))+
  geom_vline(xintercept=c(-1,1),lty=4,col="black",lwd=0.5) +
  geom_hline(yintercept = 1.301,lty=4,col="black",lwd=0.5) +
  labs(x="log2(fold change)",
       y="-log10 (adj.p-value)",
       title = expression("LuSn"^"high"*" vs NaSn"^"low"))  +
  theme_bw()+
  theme(plot.title = element_text(size = 20, hjust = 0.5, colour = "#DB72FB"), 
        axis.title=element_text(size=20),
        legend.position="right", 
        legend.title = element_blank(),
        legend.text = element_text(size=15),
...
p <- ggplot(data = test, 
            aes(x = log2FoldChange, 
                ...
                colour=expression)) +
  geom_point(alpha=0.8, size=2) +
  scale_color_manual(values=c("DEGs in NaSn+"="#00BA38", "DEGs in LuSn+"="#619CFF", "non-DEGs"="grey"),
                     labels = c(bquote("DEGs in NaSn"^"mid"), bquote("DEGs in LuSn"^"high"), "non-DEGs"))+
  geom_vline(xintercept=c(-1,1),lty=4,col="black",lwd=0.5) +
  geom_hline(yintercept = 1.301,lty=4,col="black",lwd=0.5) +
  labs(x="log2(fold change)",
       y="-log10 (adj.p-value)",
       title = expression("LuSn"^"high"*" vs NaSn"^"mid"))  +
  theme_bw()+
  theme(plot.title = element_text(size = 20, hjust = 0.5, colour = "#00798C"), 
        axis.title=element_text(size=20),
        legend.position="right", 
        legend.title = element_blank(),
        legend.text = element_text(size=15),
